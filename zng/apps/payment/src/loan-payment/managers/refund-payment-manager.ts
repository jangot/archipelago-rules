...
export class RefundPaymentManager extends BaseLoanPaymentManager {
  ...
  public async initiate(loanId: string): Promise<ILoanPayment | null> {
    this.logger.debug(`Initiating refund payment for loan ${loanId}`);
    ...
    const refundPayment = payments && payments.find(payment => payment.type === this.paymentType);
    if (refundPayment) {
      this.logger.error(`Refund payment already exists for loan ${loanId}`);
      return null;
    }
    ...
    if (!payment) {
      this.logger.error(`Failed to create refund payment for loan ${loanId}`);
    ...
    if (!generatedSteps || !generatedSteps.length) {
      this.logger.error('Failed to generate refund payment steps for loan', { payment, route, lenderAccountId, billerPaymentAccountId: biller.paymentAccountId });
  ...
  protected generateStepsForPayment(
    ...
    if (!payment) {
      this.logger.error('Failed to generate refund payment steps for loan as payment was not provided', { payment, route, fromAccountId, toAccountId });
      return null; // Payment creation failed
    }
