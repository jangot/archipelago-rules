...
- [Читать правила](rules/README.md)
- [Спросить бота](https://t.me/archipelago_rules_bot)
...
5. **Запуск в продакшене с PM2**:
```bash
# Установите PM2 глобально (если не установлен)
npm install -g pm2

# Используйте скрипт для управления ботом
./start-bot.sh start    # Запустить
./start-bot.sh stop     # Остановить
./start-bot.sh restart  # Перезапустить
./start-bot.sh status   # Статус
./start-bot.sh logs     # Логи
```
