...
### DomainService Implementations
- All DomainService implementations return the following data types (wrapped in a Promise):
  - update*, restore*, softDelete* methods return boolean | null
  - create* methods return the interface implemented by the Entity (e.g. `IApplicationUser` for `ApplicationUser` entity) | null
  - get* methods return either a single interface (e.g. `IApplicationUser`) or an array (e.g. `IApplicationUser[]`) | null
  - search* methods return an `IPaging<EntityInterface>` (e.g. `IPaging<IApplicationUser>`)
  - Other than Search and Paging related Dtos, there are no DTOs used in here
